function __wizrocket() {
    ...
    wiz.init = function () {
        ...
        dataPostURL = wz_pr + '//' + targetDomain + '/a?t=74';
    ...
    wiz.pushOutStaleEvents = function () {
        var urlStr = wiz.readCookie(STRING_CONSTANTS.LCOOKIE_NAME);
        ...
        if (urlStr && urlStr.length > 10) {
            ...
            var epoch = urlArr[1];
            var retryCount = (typeof urlArr[2] == STRING_CONSTANTS.UNDEFINED ? 0 : urlArr[2]);
            ...
            if (d > staleEvtMaxTime || retryCount >= 1) { // try once max for stale evt.
                wc.d("ign stale ck " + STRING_CONSTANTS.LCOOKIE_NAME + " w/val: " + urlStr);
            } else {
                ...
                wiz.createBroadCookie(STRING_CONSTANTS.LCOOKIE_NAME, data, TWENTY_MINS, domain); // self-destruct after 20 minutes
                wc.d("stored in " + STRING_CONSTANTS.LCOOKIE_NAME + "-> " + data);
    ...
    wiz.readFromLSorCookie = function (property) {
        ...
        if (typeof data != STRING_CONSTANTS.UNDEFINED && data !== null) {
    ...
    wiz.addFlags = function (data) {
        ...
        if ((clearCookie !== undefined && clearCookie) || blockRequeust) {
            data['rc'] = true;
        }
        if (wzrk_util.isPersonalizationActive()) {
...
// profile like https://developers.google.com/+/api/latest/people
    ...
    wiz.g = function () {
        gcookie = wiz.readCookie(STRING_CONSTANTS.GCOOKIE_NAME);

        if (gcookie == null && wzrk_util.isLocalStorageSupported()) { // only get from localStorage if cookie is null
            gcookie = localStorage[STRING_CONSTANTS.GCOOKIE_NAME];
        }
    ...
    wiz.manageSession = function (session) {
        ...
        if (typeof currentSessionId == STRING_CONSTANTS.UNDEFINED) {
    ...
    wiz.s = function (global, session, resume, respNumber) {
        ...
        // global cookie
        var cookieExpiry = 86400 * 365 * 10; //10 years in seconds. Seconds in an days * days in an year * number of years
        gcookie = global;

        wiz.createBroadCookie(STRING_CONSTANTS.GCOOKIE_NAME, global, cookieExpiry, domain);

        if (wzrk_util.isLocalStorageSupported()) {  //write to HTML 5 localstorage
            try {
                localStorage[STRING_CONSTANTS.GCOOKIE_NAME] = global;
            } catch (e) {
        ...
        wiz.deleteCookie(STRING_CONSTANTS.LCOOKIE_NAME, broadDomain);
        wc.d("del ck: " + STRING_CONSTANTS.LCOOKIE_NAME);
        if (resume) {
            wiz.pushOutStaleEvents();
        }
    ...
    wiz.isChargedEventStructureValid = function (chargedObj) {
        if (wzrk_util.isObject(chargedObj)) {
            ...
            //save charged Id
            if (typeof chargedObj[STRING_CONSTANTS.CHARGED_ID] != STRING_CONSTANTS.UNDEFINED) {
                var chargedId = chargedObj[STRING_CONSTANTS.CHARGED_ID];
                if (typeof globalChargedId == STRING_CONSTANTS.UNDEFINED) {
                    globalChargedId = wzrk_util.readFromLSorCookie(STRING_CONSTANTS.CHARGED_ID);
                }
                if (typeof globalChargedId != STRING_CONSTANTS.UNDEFINED && globalChargedId == chargedId) {
                    //drop event- duplicate charged id
                    wiz.e("Duplicate charged Id - Dropped" + chargedObj);
                    return false;
                }
                globalChargedId = chargedId;
                wzrk_util.saveToLSorCookie(STRING_CONSTANTS.CHARGED_ID, chargedId);
            }
            return true;
